...
# -*- coding: utf-8 -*-
"""
CWJ Agent v3.2 — 全自动智能编程助手（Termux）
"""
...
API_KEY = profile.get("api_key", "ark-6aaae18f-0350-4822-bbee-42e2d0655215-c73ec")
...
def push_action(original_path, backup_path):
    try:
        ...
        if os.path.exists(ACTION_STACK_FILE):
            with open(ACTION_STACK_FILE, 'r') as f:
                stack = json.load(f)
        stack.append({
        ...
        with open(ACTION_STACK_FILE, 'w') as f:
            json.dump(stack, f, indent=2)
    except Exception:
        pass

def undo_last_action():
    if not os.path.exists(ACTION_STACK_FILE):
        return "没有可回滚的操作。"
    try:
        with open(ACTION_STACK_FILE, 'r') as f:
            stack = json.load(f)
...
_last_tool = ""
_read_count = {}
_tool_stats = {"search":0,"read":0,"replace":0,"write":0,"cmd":0}
...
def search_file(filepath, keyword):
    global _last_tool, _tool_stats
    _last_tool = "search_file"
    _tool_stats["search"]+=1
...
def preview_command(cmd):
    ...
    if cmd.strip().startswith("rm"):
        return "⚠️ 此命令将删除文件或目录。建议使用 trash 或 mv 代替。"
    if cmd.strip().startswith("git push"):
        return "📤 将本地提交推送到远程仓库。"
    if cmd.strip().startswith("git commit"):
        return "💾 创建一个新提交，包含暂存区的更改。"
    if any(x in cmd for x in ["rm","mv","cp","chmod","chown"]):
        return f"⚠️ 没有内置预览，将直接执行。命令: {cmd}"
    return f"💡 将执行命令: {cmd}"
...
def generate_dynamic_system_prompt():
    base = (
        ...
        "核心规则：\n"
        "1. 优先使用文件工具（read/write/replace/patch）直接修改，仅必要时用命令。\n"
        "2. 修改前必须先用search_file定位，再用replace/patch精准修改。\n"
        "3. 支持一次调用多个独立工具（如同时读多个文件）。\n"
        "4. 修改后自动备份，可回滚。\n"
        "5. 写入和追加操作成功后无需读取确认，替换成功后也无需确认，操作失败时才需检查。\n"
        "6. 修改网站文件时，每次只改一个文件，改完立即备份，不要一次性修改多个文件。\n"
        "7. CSS修改优先用replace_in_file精准替换，不要整个文件重写。\n"
        "8. 改完网站文件后提醒用户手动执行up命令推送到GitHub。\n"
        "5. 安全第一：危险命令需确认。\n"
    )
    prefs = load_preferences()
    pref_str = (
        f"用户偏好：语言 {prefs.get('language','python')}，编码风格 {prefs.get('code_style','pep8')}，"
        f"日志 {prefs.get('logging','logging')}，注释语言 {prefs.get('comments','english')}。"
        f"{prefs.get('extra_notes','')}"
    )
    base += f"\n{pref_str}\n"

    if os.path.exists(os.path.join(WORKDIR, "Dockerfile")):
        base += "项目包含 Dockerfile，注意容器化相关建议。\n"
    if os.path.exists(os.path.join(WORKDIR, "requirements.txt")):
        base += "项目使用 Python pip 依赖管理。\n"
    exts = set()
    try:
        for f in os.listdir(WORKDIR):
            if os.path.isfile(os.path.join(WORKDIR, f)):
                ext = os.path.splitext(f)[1].lower()
                if ext in {'.py','.js','.html','.css','.json','.sh','.md','.txt'}:
                    exts.add(ext)
    except Exception:
        pass
    if '.py' in exts:
        base += "检测到 Python 文件，请遵循 PEP8 风格。\n"
    if '.js' in exts:
        base += "检测到 JavaScript 文件，尽量使用 ES6+ 语法。\n"
    return base.strip()
...
def summarize_context(messages, max_summary_tokens=150):
    ...
    conversation = "\n".join(text_parts[-10:])
    prompt = (
        "请将以下对话压缩为一段简洁的摘要，保留任务目标、当前进度、关键文件列表。"
        f"最多{max_summary_tokens}字：\n\n{conversation}"
    )
    try:
...
def smart_truncate_tool_result(tool_name, raw_result):
    if disable_truncation:
        return raw_result[:10000] if len(raw_result) > 10000 else raw_result
    if complex_mode:
...
def adjust_dynamic_params(output_len):
    ...
    if output_len >= current_max_tokens - 20:
        if current_max_tokens < 800:
            current_max_tokens = 800
        elif current_max_tokens < 1200:
...
def print_banner():
    ...
    print(c("  /undo   回滚修改  |  /prefs 查看偏好","dim"))
    print(c("  /save  保存会话   |  /resume 恢复会话  |  /clear_session 清除会话","dim"))
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if cmd == "/detail":
                ...
                print(c(undo_last_action(), "yellow"))
                continue
            elif cmd == "/prefs":
                prefs = load_preferences()
                print(c("  当前偏好：", "cyan"))
                ...
                if hist:
                    ...
                    if history and history[0]["role"] == "system":
                        ...
                        history.insert(0, {"role": "system", "content": BASE_SYSTEM})
                    saved_at = meta.get("saved_at", "未知时间")
                    print(c(f"  ⏱️ 保存时间：{saved_at}", "dim"))
                    print(c(f"  📊 当前累计 token: {total_tokens}", "dim"))
                else:
                    print(c("  📭 没有可恢复的会话", "yellow"))
                continue
            elif cmd == "/clear_session":
                print(c("  " + clear_session(), "green"))
                continue
            elif cmd == "/help":
                print(c("  /detail /normal /complex /simple /undo /prefs /save /resume /clear_session /help", "cyan"))
                continue
            else:
                print(c("  未知命令，输入 /help 查看", "yellow"))
                continue
        ...
        if session_resumed:
            session_resumed = False
            if history and history[0]["role"] == "system":
                history[0]["content"] = generate_dynamic_system_prompt()
            else:
                history.insert(0, {"role": "system", "content": generate_dynamic_system_prompt()})
        ...
        try:
            while True:
                ...
                if any(phrase in text_content for phrase in ["继续","需要更多步骤","还需进一步","尚未完成"]):
                    if max_steps < 25:
                        max_steps += 5
                        print(c(f"  ↪️  模型要求继续，步数上限提升至 {max_steps}", "yellow"))
                ...
                history.append({"role": "assistant", "content": msg.content, "tool_calls": [{"id": tc.id, "type": "function", "function": {"name": tc.function.name, "arguments": tc.function.arguments}} for tc in (msg.tool_calls or [])] if msg.tool_calls else []})
                tool_results = []
                print()
                ...
                for tool in msg.tool_calls:
                    ...
                    if name == "run_command":
                        ...
                        res = append_file(args["filepath"], args["content"])
                    elif name == "replace_in_file":
                        res = replace_in_file(args["filepath"], args["old_text"], args["new_text"])
                    elif name == "patch_file":
                        res = patch_file(args["filepath"], args["patches"])
                    elif name == "search_file":
                        res = search_file(args["filepath"], args["keyword"])
                    elif name == "list_dir":
                    ...
                    tool_results.append({"role": "tool", "tool_call_id": tool.id, "content": stored})
